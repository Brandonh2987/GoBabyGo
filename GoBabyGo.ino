...
int xLimitHigh = 512+SPEED_LIMIT  ; //512+128
int xLimitLow = 512-SPEED_LIMIT   ; //512-128
int yLimitHigh = 512+SPEED_LIMIT  ;
int yLimitLow = 512-SPEED_LIMIT   ;
...
void loop() {
  ...
  int x = -1 * analogRead(JOYSTICK_X) + 1023;  // 250 to 520 to 1275 --> 0 to 1023
    // 250+250 = 500 --> 512
    // 0 + 250 = 250 (full left) --> 0
    // 1023 + 250 = 1275 (full right) --> 1023
    
  int y =  analogRead(JOYSTICK_Y) + yOffset;
  debug("RawX:", x);
  debug("RawY:", y);
...
//  x = map(x, 0, 1023, xLimitLow, xLimitHigh);
//  y = map(y, 0, 1023, yLimitLow, yLimitHigh);
  debug("Xout:", x);
  debug("Yout:", y);
